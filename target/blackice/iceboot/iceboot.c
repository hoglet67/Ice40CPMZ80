...
#include "cpmz80_roms.h"
#define  CPMZ80_ROMS_START 0x000000

#define VER "0.35 Ice40CPMZ80"
...
void send_cpmz80_roms() {
   uint32_t start = CPMZ80_ROMS_START;
   uint32_t end   = CPMZ80_ROMS_START + cpmz80_roms_bin_len - 1;

	// Additional code to send the Cpmz80 ROMs (16KB total)
	gpio_high(ICE40_SPI_CS);
	HAL_Delay(100);
	uart_puts("Sending CPMZ80 ROMS\n");
	gpio_low(ICE40_SPI_CS);
	spi_write((uint8_t *)&start, 3);
	spi_write((uint8_t *)&end, 3);
	spi_write((uint8_t *)&cpmz80_roms_bin[0], cpmz80_roms_bin_len);
	gpio_high(ICE40_SPI_CS);
	uart_puts("Done\n");
}
